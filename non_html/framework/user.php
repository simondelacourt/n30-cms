...
class user
{
	...
	{
		$QE = $this->cms->connection->query("SELECT id FROM `" . TAB_USERS . "` WHERE username = '" . $this->cms->connection->escape_string($username) . "' AND pass = sha1('" . $this->cms->connection->escape_string($password) . "')");
